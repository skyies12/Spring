...
%>
      <div id="wrap">
       <!-- header -->
		...
		<form id="select" action="select?bNum=<%=bNum %>" method="post">
		...
		<tr>
			...
			<div class="btn-group mr-2" role="group" aria-label="First group">
				...
				<c:choose>
					...
					<c:otherwise>
						<a class="btn btn-secondary" href="select?bNum=<%= bNum %>&page=1&serchcheck=<%=selectNum%>&bTitle=<%=bTitle %>">처음</a>
					</c:otherwise>
				</c:choose>
				<!-- 이전 -->
				<c:choose>
					...
					<c:otherwise>
						<a class="btn btn-secondary" href="select?bNum=<%= bNum %>&page=${page.curPage - 1}&serchcheck=<%=selectNum%>&bTitle=<%=bTitle %>">&lt; 이전</a>
					</c:otherwise>
				</c:choose>
				</div>
				<!-- 개별 페이지 -->
				...
				<c:forEach var="fEach" begin="${page.startPage }" end="${page.endPage }" step="1">
					<c:choose>
						...
						<c:otherwise>
							<a class="btn btn-secondary" href="select?bNum=<%= bNum %>&page=${fEach }&serchcheck=<%=selectNum%>&bTitle=<%=bTitle %>">${fEach }</a>
						</c:otherwise>
					</c:choose>
				</c:forEach>
				</div>
				...
				<c:choose>
					...
					<c:otherwise>
						<a class="btn btn-secondary" href="select?bNum=<%= bNum %>&page=${page.curPage + 1}&serchcheck=<%=selectNum%>&bTitle=<%=bTitle %>">다음 &gt;</a>
					</c:otherwise>
				</c:choose>
				...
				<c:choose>
					...
					<c:otherwise>
						<a class="btn btn-secondary" href="select?bNum=<%= bNum %>&page=${page.totalPage }&serchcheck=<%=selectNum%>&bTitle=<%=bTitle %>">끝</a>
					</c:otherwise>
				</c:choose>
				</div>
				</div>
